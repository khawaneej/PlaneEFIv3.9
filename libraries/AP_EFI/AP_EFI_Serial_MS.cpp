...
{
    _internal_state.estimated_consumed_fuel_volume_cm3 = 0; // Just to be sure
...
void AP_EFI_Serial_MS::update()
{
    if (_port) {
        ...
        uint32_t buffered_data_size = _port->available();
...
bool AP_EFI_Serial_MS::read_incoming_realtime_data()
{
    ...
    received_CRC = _port->read() << 24;
    received_CRC += _port->read() << 16;
    received_CRC += _port->read() << 8;
    ...
    _internal_state.last_updated_ms = current_time;
...
void AP_EFI_Serial_MS::send_request(uint8_t table, uint16_t first_offset, uint16_t last_offset)
{
    ...
    // Write the CRC32
    _port->write((uint8_t)(crc >> 24));
    _port->write((uint8_t)(crc >> 16));
    _port->write((uint8_t)(crc >> 8));
    _port->write((uint8_t)crc);
...
uint8_t AP_EFI_Serial_MS::read_byte_CRC32()
{
    ...
    _checksum = CRC32_compute_byte(_checksum, data);
    return data;
}


// CRC32: based on public-domain example specified in the MegaSquirt documentation
// http://csbruce.com/software/crc32.c
uint32_t AP_EFI_Serial_MS::CRC32_compute_byte(uint32_t inCRC32, uint8_t data)
{
   ...
   0xB40BBE37,0xC30C8EA1,0x5A05DF1B,0x2D02EF8D };
    ...
    // Update the CRC32 for the newest byte
    crc32 = inCRC32 ^ 0xFFFFFFFF;
    crc32 = (crc32 >> 8) ^ crcTable[ (crc32 ^ data) & 0xFF ];
